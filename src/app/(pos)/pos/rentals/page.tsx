...
function EmptyHint({ text }: { text: string }) {
  return (
    <div className="rounded-2xl bg-white px-4 py-5 text-center text-[13px] text-zinc-400 ring-1 ring-zinc-100">
      {text}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-2">
      {Array.from({ length: 2 }).map((_, i) => (
        <div
          key={i}
          className="flex items-center gap-3 rounded-2xl bg-white p-3 ring-1 ring-zinc-100"
        >
  ...
  const inner = (
    <>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <span className="line-clamp-1 text-[12px] text-zinc-600">
