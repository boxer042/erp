 ...
 * 풀스크린 모달, 결과 카테고리별 섹션.
 */
export function GlobalSearchSheet({ open, onOpenChange }: Props) {
  if (!open) return null;
              ...
              <Section title="상품" count={products.length}>
                {products.slice(0, 10).map((p) => (
                  <Row
                    ...
                    onClick={() => {
                      close();
                      router.push(`/pos/v2/product/${p.id}`);
                    }}
                  >
                ...
                {customers.map((c) => (
                  <Row
                    ...
                    onClick={() => {
                      ...
                      if (existing) {
                        router.push(`/pos/v2/customer/${existing.id}`);
                      } else {
                        const sid = addSession();
                        if (sid) {
                          setCustomer(c.id, c.name, c.phone, sid);
                          router.push(`/pos/v2/customer/${sid}`);
                        }
                      }
                    }}
                  >
