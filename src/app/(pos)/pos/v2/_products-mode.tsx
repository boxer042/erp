 ...
 */
export function ProductsMode({ session, onPrintLabels }: Props) {
      ...
      <header className="shrink-0 overflow-hidden border-b border-zinc-100 bg-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <button
            ...
            className="my-2 flex h-11 w-full items-center gap-2 rounded-xl bg-zinc-100 px-4 text-left text-[14px] text-zinc-500 hover:bg-zinc-200"
          >
              ...
              <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 sm:gap-3 md:grid-cols-4">
                {(popularQuery.data ?? []).map((p) => (
                  <ProductGridCard
                    ...
                    onDetail={() => router.push(`/pos/v2/product/${p.id}`)}
                  />
                ))}
              </div>
