...
export function ProductGridCard({ product, onClick, onDetail }: Props) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      onKeyDown={onKey}
      className="group relative flex cursor-pointer flex-col overflow-hidden rounded-2xl bg-white text-left ring-1 ring-zinc-200 transition-all active:scale-[0.98] sm:hover:ring-zinc-300"
    >
      {/* ℹ️ 상세 버튼 — 우상단 */}
      {onDetail && (
        <button
          ...
          className="absolute right-1.5 top-1.5 z-10 flex h-7 w-7 items-center justify-center rounded-full bg-white/95 text-zinc-700 shadow ring-1 ring-zinc-200 backdrop-blur active:scale-95 sm:hover:bg-white"
          aria-label="상세 보기"
        >
      ...
      <div className="relative aspect-square w-full overflow-hidden bg-zinc-50">
        ...
        {(taxFree || product.zeroRateEligible) && (
          <span className="absolute left-2 top-2 inline-flex items-center rounded-full bg-white/95 px-2 py-0.5 text-[10px] font-medium text-zinc-700 backdrop-blur">
            {taxFree ? "면세" : "영세"}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-0.5 p-2.5 sm:p-3">
        <span className="line-clamp-2 min-h-[2.4em] text-[12px] font-semibold leading-tight text-zinc-900 sm:text-[13px]">
          {product.name}
        </span>
        <span className="font-mono text-[10px] text-zinc-400">
          {product.sku}
        </span>
        <span className="mt-1 text-[14px] font-bold tabular-nums text-zinc-900 sm:text-[15px]">
          {autoNoPrice ? (
