    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const selected = await tx.product.findUnique({
        ...
        select: {
          ...
          // 변형은 설계상 canonical 과 같은 상품 (구성품 일부만 다름) — 시리얼·보증·브랜드·이미지 등
          // 본질 식별 필드 모두 복사해야 카탈로그·POS·인벤토리에서 동등하게 동작.
          trackable: true,
          warrantyMonths: true,
          warrantyPolicy: true,
          brand: true,
        ...
        if (matched) {
          ...
          const newVariant = await tx.product.create({
            data: {
              ...
              // canonical 본질 식별 필드 복사 — variant 는 같은 상품의 다른 조립 구성일 뿐.
              trackable: selected.trackable,
              warrantyMonths: selected.warrantyMonths,
              warrantyPolicy: selected.warrantyPolicy,
              ...
              zeroRateEligible: selected.zeroRateEligible,
              countryOfOrigin: selected.countryOfOrigin,
              manufacturer: selected.manufacturer,
              importer: selected.importer,
              certifications: selected.certifications,
              asResponsible: selected.asResponsible,
              catalogHidden: selected.catalogHidden,
