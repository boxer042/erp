...
export async function POST(request: NextRequest) {
  ...
  if (!ALLOWED_MIME.includes(file.type)) {
    return NextResponse.json({ error: "JPG/PNG/WebP/GIF/SVG만 지원합니다" }, { status: 400 });
  }
  ...
  if (error) {
    console.error("[products/upload] supabase upload error", error);
...
export async function DELETE(request: NextRequest) {
  ...
  if (error) {
    console.error("[products/upload] supabase delete error", error);
