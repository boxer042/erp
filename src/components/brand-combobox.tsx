    ...
    <Popover open={open} onOpenChange={(v) => { if (v) setSearch(""); setOpen(v); }}>
      <PopoverTrigger className="relative flex h-9 max-h-9 box-border w-full items-center overflow-hidden rounded-lg border border-input bg-transparent pl-3 pr-9 text-sm cursor-pointer hover:bg-accent/50 focus:outline-none focus-visible:outline-none">
        {selected?.logoUrl && (
          ...
          <img src={selected.logoUrl} alt="" className="mr-2 size-5 shrink-0 rounded object-contain bg-card border border-border" />
        )}
            ...
            <CommandGroup>
              {filtered.map((b) => (
                <CommandItem
                  key={b.id}
                  value={b.id}
                  onSelect={() => { onChange(b.id, b.name); setOpen(false); setSearch(""); }}
                  data-checked={b.id === value ? "true" : undefined}
                >
                  {b.logoUrl && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={b.logoUrl} alt="" className="h-4 w-4 rounded object-contain bg-card border border-border" />
                  )}
                  <span>{b.name}</span>
                </CommandItem>
              ))}
