...
export function ProductVariantsCard({ productId, taxType, variants, parentSetComponentsEmpty }: ProductVariantsCardProps) {
  ...
  const archiveVariant = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(typeof err.error === "string" ? err.error : "비활성 실패");
        return;
      }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        toast.error(typeof err.error === "string" ? err.error : "일괄 수정 실패");
        return;
      }
