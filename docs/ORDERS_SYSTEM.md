...
- rate limit 대응
- webhook signature 검증 (각 채널 별 방식)

**Phase 3 — 자동 트리거 인프라 (부분 완료, 2026-05-07)**
- ✓ **Polling cron 라우트**: `/api/cron/channel-poll` (Bearer CRON_SECRET 인증). 활성 채널 + registry 어댑터 등록된 것 모두 자동 import. Vercel cron 또는 외부 스케줄러 (Supabase pg_cron, GitHub Actions, cron-job.org) 등록만 남음
- 남은 작업: **Webhook 라우트** scaffolding (`/api/webhooks/channel/[provider]`). signature 검증은 Phase 2 후에야 채널별 구체화 가능
- 남은 작업: **실패 retry 큐** (PendingChannelOrder.status=VALIDATION_FAILED 활용 또는 별도 ChannelOutboundJob 모델)

**Phase 4 — Outbound 동기화 (부분 완료, 2026-05-07)**
- ✓ ERP `[발송]` 액션 직후 `adapter.pushTrackingNumber()` 자동 호출 (best-effort, 실패해도 ERP 액션 영향 X, audit log 기록)
- ✓ ERP `[수락]/[반려]` 직후 `adapter.acceptReturn()/rejectReturn()` 자동 호출
- 남은 작업: **자동 retry 큐** — 현재는 1회 시도 후 실패 시 audit log 만. ChannelOutboundJob 큐 + cron 으로 backoff retry
...
- 모두 Phase 2 후 채널 webhook payload schema 확정 후 구현 가능

**Phase 6+ — 운영 효율화 (부분 완료, 2026-05-07)**
- ✓ **SKU 자동 매핑 추천** — 보류 큐의 UNMAPPED_SKU 행에서 [추천] 클릭 시 dialog 로 ERP 상품 후보 list (점수순). 1클릭 매핑. 알고리즘은 SKU 일치 + 상품명 토큰 일치
- ✓ **운영 대시보드 위젯** — `/channels/imports` 상단에 보류 누적/최근 7일 import·변환 카운트 + 자주 누락 SKU top 5 chip
- 남은 작업: **재고 sync** — Inventory 변동 시 채널에 가용 재고 push. 채널별 단가도 sync
- 남은 작업: **다중·variant 매핑** — 한 채널 SKU 가 ERP 세트 상품 또는 variant 로 매핑 (현재 1:1만)
- 남은 작업: **임계값 알림** — 보류 누적 N건 이상이면 매장에 알림 (알림 시스템 선결)
- 남은 작업: **채널 설정 페이지** — credentials, 정책(D+1, 운임), polling 빈도 매장 직접 관리
...
- **2026-05-07 (5차)**: 외부 채널 자동화 Phase 2-비의존 후속 작업 — SKU 자동 매핑 추천 (점수순 추천 + 1클릭 매핑 dialog), Outbound hook (ERP `[발송]/[수락]/[반려]` 직후 adapter 자동 호출, best-effort), Cron 라우트 (`/api/cron/channel-poll`, Bearer CRON_SECRET), 운영 대시보드 위젯 (보류·import·누락 SKU top 통계).
